...
- 2026-06-08: Continued P0 Codex credential-boundary hardening. Repository implementation, PR review-fix, cleanup and preview-source manifests now expose reusable Codex auth material only through `ROSENVALL_CODEX_LAUNCHER_HOME`; the runner helper creates a per-run `codex-runtime-home`, clears launcher files before Codex execution and clears runtime auth files after startup/exit. The full broker or split-container boundary is still the remaining closure criterion.
...
- 2026-06-07: Continued ticket 7 Source/provider-sync module split. Provider-sync action-key normalization, quota policy, source/target provider comparison, sync repository create/link/run request builders and provider-sync operation-kind constants now live in `RepositorySourceFeature`, leaving the endpoint focused on auth, provider credentials, external repository creation, runtime Secret writes and Kubernetes Job submission.
- 2026-06-07: Continued ticket 7 Source/provider-sync module split by moving `RepositorySourceFeature` into `src/Rosenvall.DevOps.Api/Features/Source/RepositorySourceFeature.cs` and extracting board source repository DTO projection out of the `/boards/{boardId}/source/repositories` endpoint. Source read endpoints and provider-sync submission still need full feature route/handler extraction.
- 2026-06-07: Continued ticket 7 Source/provider-sync module split by moving Source read result/error mapping into `RepositorySourceFeature.ReadResultAsync`, so provider rejected requests, bad JSON and transport failures are handled by the Source feature module instead of a top-level `Program.cs` helper.
- 2026-06-07: Continued ticket 7 Source/provider-sync module split by moving clone-info API response mapping into `RepositorySourceFeature.BuildCloneInfoDto`, leaving the clone-info endpoint responsible only for authorization, repository lookup and returning the feature-built response.
- 2026-06-07: Continued ticket 7 Source/provider-sync module split by moving board Source repository, tree, file and clone-info route mappings into `RepositorySourceEndpoints`, leaving `Program.cs` to register the feature module while provider-sync submission remains the next extraction boundary.
- 2026-06-07: Continued ticket 7 Source/provider-sync module split by moving provider-sync submission into `RepositorySourceEndpoints` as well. Source tree/file/clone and sync-to-provider routes now register through the Source feature module, while provider-sync renderer/client internals remain separate follow-up extraction candidates.
- 2026-06-07: Closed the Source ref-input request-churn finding. `SourceView` now keeps a draft ref separate from the committed ref used by tree/file requests, commits on blur/Enter, aborts superseded requests, and has frontend regression coverage so typing in the ref field does not fetch provider data on every keystroke.
- 2026-06-07: Continued the `Program.cs` module split with realtime delivery. `DevOpsHub`, `IRealtimeNotifier`, `RealtimeNotifier` and `RealtimeMode` now live under `src/Rosenvall.DevOps.Api/Realtime/RealtimeNotifications.cs`, with regression coverage ensuring scoped SignalR sends stay out of `Program.cs` and no `Clients.All` broadcasts return.
- 2026-06-07: Continued the `Program.cs` module split with authentication configuration. `AuthenticationMode` now lives under `src/Rosenvall.DevOps.Api/Auth/AuthenticationMode.cs`, preserving the fail-closed startup behavior while removing another startup policy block from `Program.cs`.
- 2026-06-07: Continued the `Program.cs` module split with browser/webhook security policy. `GitHubWebhookSignatureVerifier` and `BrowserSecurityHeaders` now live under `src/Rosenvall.DevOps.Api/Security/`, with regression coverage keeping signature verification and response header policy out of `Program.cs`.
- 2026-06-07: Continued the `Program.cs` module split with API contracts. Public DTOs, request records and action-ledger block reason constants now live under `src/Rosenvall.DevOps.Api/Contracts/ApiContracts.cs`, with regression coverage preventing the contract block from returning to `Program.cs`.
- 2026-06-07: Continued the `Program.cs` module split with operations diagnostics. `ApiResourceDiagnosticsReader` and `ImplementationCapacityPreflight` now live under `src/Rosenvall.DevOps.Api/Operations/ResourceDiagnostics.cs`, with regression coverage keeping API memory diagnostics and implementation preflight policy out of `Program.cs`.
- 2026-06-07: Closed the OIDC browser-token storage slice. The frontend `oidc-client-ts` user store now uses `window.sessionStorage` instead of persistent `window.localStorage`, while non-sensitive UI preferences remain local-storage backed, and a frontend regression test keeps auth tokens out of persistent browser storage.
- 2026-06-07: Continued the runtime/Kubernetes split by moving `KubernetesFailureClassifier` and `CodexKubernetesRunner` into `src/Rosenvall.DevOps.Api/Runtime/KubernetesRunnerSupport.cs`, with regression coverage keeping runner defaults and Kubernetes failure classification out of `Program.cs`.
- 2026-06-07: Continued the runtime/Kubernetes split by moving the generic `PipelineJobManifestRenderer` into `src/Rosenvall.DevOps.Api/Runtime/PipelineJobManifestRenderer.cs`, with regression coverage keeping generic pipeline Job manifest rendering out of `Program.cs`.
...
- 2026-06-07: Closed the frontend build-SHA follow-up for deployment drift visibility. Vite now embeds `VITE_RELEASE_COMMIT_SHA` and `VITE_RELEASE_BUILD_TIMESTAMP` into the frontend bundle, the frontend Docker image receives those values from the publish workflow, and Settings shows both frontend bundle identity and frontend image identity before comparing them with API release diagnostics.
...
- 2026-06-07: Continued ticket 8 typed manifest assertions for runner Jobs. Implementation, PR review-fix, repository cleanup and preview-source manifest tests now parse Job YAML and assert container/initContainer images, service-account token settings, Codex-home volumes/mounts, token `secretKeyRef` env vars and auth-file cleanup command ordering structurally instead of relying only on string fragments.
...
- 2026-06-07: Continued ticket 9 by extending the action ledger to AI plan and AI plan revision starts. Both endpoints now reserve a server-derived idempotency key before calling the AI provider, reject duplicate in-flight requests with `409`, record created AI runs on the ledger, mark provider/start failures, and hash revision free text instead of storing it in the ledger key. This covers duplicate-click/browser retry protection for active AI provider calls; per-actor quotas plus idempotency coverage for preview retry, implementation, PR review fix, cleanup and repository creation remain follow-up work.
- 2026-06-07: Added the first per-actor quota slice for ticket 9. `DevOpsStore.StartAction` can now enforce a time-window quota after idempotency checks but before creating a new ledger entry, and AI plan/revise uses a shared `Actions:Quotas:AiPlanning` policy that returns `429` with retry metadata when an actor exceeds the configured AI planning allowance. Quota coverage still needs to be extended to provider sync, preview retry, implementation, PR review fix, cleanup and repository creation.
- 2026-06-07: Extended ticket 9 quota coverage to provider sync. Repository copy-to-provider now uses `Actions:Quotas:ProviderSync` through the same `ActionLedger` start path, so duplicate sync requests still return the existing operation while distinct new sync attempts over the per-actor window return `429` before creating target repositories, runtime token Secrets or Kubernetes Jobs. Remaining quota/idempotency coverage gaps are preview retry, implementation, PR review fix, cleanup and repository creation.
- 2026-06-07: Extended ticket 9 quota/idempotency coverage to direct repository implementation starts. `POST /work-items/{id}/implementation-runs` now reserves a server-derived `repository-implementation` action before creating the implementation run, returns `409` for duplicate pre-run starts, returns `429` via `Actions:Quotas:RepositoryImplementation` for over-quota actors, records the created run on the action ledger, and marks the action failed when pre-job setup fails. Failed implementation retries remain allowed through the existing retry path. Remaining quota/idempotency gaps are preview retry, PR review fix, cleanup and repository creation.
- 2026-06-07: Extended ticket 9 quota/idempotency coverage to LocalGit pull-request review fix runs. `POST /work-items/{id}/pull-request/ai-fix-comments` now reserves a `pr-review-fix` action before queuing the Codex/Kubernetes fix run, returns duplicate pre-run starts as `409`, applies `Actions:Quotas:PullRequestReviewFix` for actor throttling, records queued/running run ids on the action ledger, and marks setup failures on the same ledger entry while preserving failed-run retry behavior. Remaining quota/idempotency gaps are preview retry, cleanup and repository creation.
- 2026-06-07: Extended ticket 9 quota/idempotency coverage to preview build starts. `POST /ai-runs/{id}/approve` now resolves the target work item before approval, uses a work-item-scoped `preview-build` action key, blocks duplicate starts while a preview is implementing/applying/provisioning, applies `Actions:Quotas:PreviewBuild`, and records the approved AI run on the ledger once preview generation is queued. Existing rebuild behavior after a non-pending preview remains allowed through the same approved plan. Remaining quota/idempotency gaps are cleanup and repository creation.
- 2026-06-07: Completed the tracked ticket 9 quota/idempotency gaps for cleanup and repository creation. Board/work-item cleanup now reserve cleanup actions before Kubernetes/Forgejo/GitHub side effects, repository cleanup PR starts are attached to the same action ledger, and LocalGit/GitHub new-repository creation share a `repository-creation` quota/idempotency policy that blocks duplicate create requests before provider calls.
...
- 2026-06-07: Hardened the delivery audit request contracts. Public delivery mutation request DTOs no longer expose `Actor`, `ApprovedBy` or `DiscardedBy` properties for implementation starts, PR review fixes, AI approval/discard, preview actions, PR approval, cleanup, cleanup adoption and pipeline execution; server-side actor values are passed explicitly from authenticated claims into store methods.
...
- 2026-06-07: Added static regression coverage for the dependency vulnerability CI gate. Backend tests now assert NuGet vulnerability audit runs after restore and before backend tests, and npm production audit runs after install and before frontend test/build.
...
- 2026-06-07: Completed the operator documentation drift slice for LocalGit. README, Homelab deploy notes, frontend control inventory and implementation checklist now describe internal-only LocalGit/Forgejo, RDO-native local pull request review/merge/deploy, Source clone guidance and provider copy instead of the older external Forgejo/link-first flow.
...
- 2026-06-07: Completed the backend SignalR scoped-delivery slice. API publishers now route through `IRealtimeNotifier`, board-scoped events are sent to authorized board groups, workspace creation events use a user-specific group, and `Clients.All` broadcasts are covered by regression tests. Frontend SignalR consumption and targeted state patching remain separate follow-up work.
- 2026-06-07: Started frontend SignalR consumption. The board UI now lazy-loads a SignalR client, subscribes to the active board/user groups, patches board cards for scoped work-item changes/deletes, refreshes only the open modal for run/comment/preview events, and keeps existing polling as a fallback for pending runtime work.
- 2026-06-07: Started the shared accessible modal primitive slice. `ModalFrame` now restores focus to the opener, closes on Escape, and cycles Tab focus inside the active dialog through a tested shared focus helper; a full component extraction and richer tab primitive remain follow-up work.
- 2026-06-07: Started the backend test-file split by moving Source feature and provider-sync module tests from `DevOpsStoreTests.cs` into `RepositorySourceFeatureTests.cs`. Store-fixture-heavy provider-sync state tests remain in the store test file until the shared test fixture is extracted.
- 2026-06-07: Continued the shared accessible modal primitive slice with keyboard-accessible work-item tabs. The card modal tablist now exposes ARIA tab semantics, roving tab index, ArrowLeft/ArrowRight/Home/End navigation and labelled tab panels backed by frontend regression tests.
- 2026-06-07: Completed the public app static serving slice. Preview manifests still run generated React source with the Vite dev server for fast feedback, while board public app manifests now build reviewed source once in an init container and serve the built `dist` output from the unprivileged nginx image instead of running `npm run dev` in production.
...
Status 2026-06-08: Partially mitigated, still open for the stronger architecture boundary. Implementation, PR review-fix, cleanup and preview-source no longer carry their own inline Codex process wrappers; all Kubernetes Codex phases go through `rdo_run_codex_without_repository_credentials`. That helper unsets repository credentials, launches the Codex command under `env -i` with an explicit allowlist for `CODEX_HOME`, model/session fields and non-secret process basics, and uses a launcher/runtime-home split: manifests copy reusable Codex auth material into `ROSENVALL_CODEX_LAUNCHER_HOME`, the helper creates a per-run `codex-runtime-home`, removes reusable launcher files before Codex execution, then clears runtime auth files after startup and exit. The prompt-driven container no longer receives `CODEX_HOME=/app/codex-home` as normal reusable home state, and unexpected board/repository environment variables no longer flow into the Codex process by default. This remains a mitigation rather than closure because the recommended broker or split-container boundary, where the prompt-driven worker never shares a container filesystem with reusable auth material at all, is still unimplemented.
...
Status 2026-06-08: Started. `Features/Source` was already split out, including provider-sync endpoints. LocalGit pull-request diff/review-comment/AI-fix endpoints now live in `Features/LocalPullRequests/LocalPullRequestEndpoints.cs`, AI session/plan/review/approve/discard endpoints live in `Features/AiPlanning/AiPlanningEndpoints.cs`, normal work-item comment endpoints now live in `Features/WorkItems/WorkItemCommentEndpoints.cs`, Environment board-secret endpoints now live in `Features/Environment/BoardSecretEndpoints.cs`, team plus board-team access endpoints now live in `Features/Teams/TeamEndpoints.cs`, workspace/board base endpoints now live in `Features/Boards/BoardEndpoints.cs`, and repository metadata/board-link endpoints now live in `Features/Repositories/RepositoryEndpoints.cs`. `Program.cs` is reduced to feature registrations for those slices, and regression coverage verifies the moved routes stay out of the top-level file. The finding remains open because Local PR approval/merge/deploy services, broad work-item CRUD/cleanup endpoints, provider-backed repository creation endpoints, DTOs, store implementation, manifest renderers and background runtime types still share `Program.cs`.
...
Status 2026-06-08: Closed for provider-sync runtime tokens. Provider-sync Jobs and token Secrets are now registered as `RuntimeArtifactDto` records in the snapshot when a sync run is queued, old provider-sync runs are backfilled into the registry on snapshot load, board/work-item cleanup renders provider-sync deletes from the registry when present, and `RuntimeArtifactCleanupReconciler` periodically deletes sensitive runtime artifacts, such as provider-sync token Secrets, after their owning pipeline run reaches a terminal state. Regression coverage verifies terminal provider-sync Secrets become cleanup candidates, deleted artifacts stop reappearing, and board cleanup uses registered artifact names instead of rediscovering provider-sync renderer names.

Evidence:

- Provider-sync token Secret names are generated by `RepositoryProviderSyncJobManifestRenderer.TokenSecretName(run)`.
- Provider-sync token Secrets are now represented as sensitive `RuntimeArtifactDto` records with owning board/work item/run metadata.
- `RuntimeArtifactCleanupReconciler` deletes sensitive runtime artifacts when the owning provider-sync run is terminal.

Impact:

- Provider-sync token Secrets no longer depend only on explicit board cleanup; terminal runs are reconciled periodically.
- Board/work-item cleanup uses the registry for provider-sync runtime artifacts when registry data exists, with legacy fallback for older snapshots.

Recommended fix:

- Keep provider-sync runtime artifacts in the first-class `RuntimeArtifactDto` record:
  - kind,
  - namespace,
  - name,
  ...
  - created timestamp,
  - sensitive flag.
- Keep cleanup deleting from this registry rather than rediscovering provider-sync names from renderers.
- Continue extending the registry to other runner families as those cleanup paths are hardened.
...
Status 2026-06-07: Closed for current release diagnostics. `/api/status` now exposes `ReleaseDiagnosticsDto` with API version, commit SHA, build timestamp, API image, frontend image, runner image and configuration mode. The frontend bundle embeds its own commit SHA and build timestamp through Vite release metadata, Settings shows both bundle and image identity, and mismatch warnings compare the frontend bundle SHA against the API SHA before falling back to frontend image identity. `scripts/doctor-local-demo.ps1` checks local API health, frontend health, Authentik mode, Forgejo port-forward, kubeconfig, runner image and optional Source/diff endpoint availability.
...
- `preview-base/Dockerfile` used to run `npm install` from `preview-base/package.json` without a lockfile; it now uses committed `preview-base/package-lock.json` and `npm ci`.
...
Status 2026-06-07: Closed. LocalGit implementation and preview-promotion runners now capture Forgejo PR creation response bodies plus HTTP status, fail explicitly on non-2xx responses with sanitized `RDO_FAILURE` output, and parse PR number/state/url with `jq` instead of sed.
...
Status 2026-06-07: Closed. `CorsConfiguration` now validates explicit frontend origins outside Development, rejects wildcard credentials and non-origin values, logs the resolved allowed origins at startup, and returns the active CORS diagnostics through `/api/status`.
