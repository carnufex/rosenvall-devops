...
- 2026-06-07: Started the shared .NET build-policy slice. Root `Directory.Build.props` now centralizes nullable reference types, implicit usings, latest analyzer level and deterministic builds for all backend projects. `.editorconfig` and selected warnings-as-errors remain follow-up work because the current test project still emits many nullable warnings.
...
Status 2026-06-07: Started. Root `Directory.Build.props` now sets shared nullable reference types, implicit usings, `AnalysisLevel=latest` and deterministic builds, with regression coverage. `.editorconfig` and narrow warnings-as-errors ratcheting remain open.
