...
| Work item modal | Approve plan | Implemented | Visible only for a `PlanReady` AI run. |
| Work item modal | Discard plan | Implemented | Marks the plan as discarded and clears active AI state. |
| Work item modal | Approve plan | Implemented | Approves the plan and starts the local nginx implementation runner for the v1 hello-world example. |
| Work item modal | Open demo environment | Implemented | Opens the preview URL returned by the backend when a preview exists. |
...
| Work item modal | Development tools / Simulate GitHub callback | Removed from v1 | Removed from the user flow because approval now produces a real local nginx preview result. |
