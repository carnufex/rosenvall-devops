...
test('shell board selection can choose boards outside the first visible workspace', () => {
  const workspaces = [
    { id: 'workspace-a', name: 'Workspace A' },
    { id: 'workspace-b', name: 'Workspace B' }
  ];
  const boardsByWorkspace = new Map([
    ['workspace-a', []],
    ['workspace-b', [{ id: 'board-b', name: 'Invited board' }]]
  ]);

  assert.deepEqual(buildShellBoardSelection(workspaces, boardsByWorkspace, 'board-b'), {
    workspace: workspaces[1],
    boards: [{ id: 'board-b', name: 'Invited board' }],
    board: { id: 'board-b', name: 'Invited board' }
  });
});
...
test('work item identity metadata keeps title clean and hierarchy editable', () => {
  assert.equal(workItemModalTitle({ key: 'TASK-4838', title: 'Initial implementation' }), 'Initial implementation');
  assert.deepEqual(workItemMetadataSummary({
    key: 'TASK-4838',
    type: 'Feature',
    isBug: true,
    parentTitle: 'Epic: Delivery UI'
  }), {
    key: 'TASK-4838',
    label: 'TASK-4838 - Feature - Bug - Epic: Delivery UI',
    parentLabel: 'Epic: Delivery UI'
  });
  assert.deepEqual(workItemMetadataSummary({ key: 'TASK-4839', type: 'Task', isBug: false }), {
    key: 'TASK-4839',
    label: 'TASK-4839 - Task - No parent',
    parentLabel: 'No parent'
  });
});

test('epic goal active status includes explicit state machine phases and legacy running states', () => {
  assert.equal(isActiveEpicGoalStatus('Running'), true);
  assert.equal(isActiveEpicGoalStatus('Queued'), true);
  assert.equal(isActiveEpicGoalStatus('PlanningChildren'), true);
  assert.equal(isActiveEpicGoalStatus('RunningChildren'), true);
  assert.equal(isActiveEpicGoalStatus('WaitingForReview'), true);
  assert.equal(isActiveEpicGoalStatus('Blocked'), false);
  assert.equal(isActiveEpicGoalStatus('Complete'), false);
  assert.equal(isActiveEpicGoalStatus('Cancelled'), false);
});
